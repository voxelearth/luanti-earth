...
/* --------------------------------------------------------------- */
export async function voxelizeModel({ model, resolution = 200, needGrid = false, method = '2.5d-scan', onStart, preRotateYDeg = 0 }) {
  return new Promise(async (resolve, reject) => {
    ...
    if (isNode) {
      ...
      const workerPath = fileURLToPath(new URL('./voxelizer.worker.js', import.meta.url));
    ...
    function handleMessage(data) {
      const { status, result, message, stack } = data;
      worker.terminate();
